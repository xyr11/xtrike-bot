...
module.exports = new BotCmd('reactionsnipe')
  .setCategory('General')
  .setThumbnail('https://imgur.com/dRSYp1f.png')
  .setDescription('Get the most recently removed reactions{{.\n[Graciously given by Dank Memer <3](https://github.com/DankMemer/sniper)}}')
  .setUsage('`$$reactionsnipe [number] [channel]`')
  .setOptionText(`\`[number]\`: get the *nth* removed reaction, default is \`1\` (newest), max is \`${maxSnipes}\` (oldest) \n\`[channel]\`: channel to snipe`)
  .setAliases(['reactsnipe'])
  .setSimilar('`$$snipe` `$$editsnipe`')
  .requiredPerm('User')
  .isDank()
  .applicationOptions([
