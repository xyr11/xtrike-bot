...
module.exports = new BotCmd('editsnipe')
  .setCategory('General')
  .setThumbnail('https://imgur.com/dRSYp1f.png')
  .setDescription('Get the original message of the most recently edited messages{{.\n[Graciously given by Dank Memer <3](https://github.com/DankMemer/sniper)}}')
  .setUsage('`$$editsnipe [number] [channel]`')
  .setOptionText(`\`[number]\`: get the *nth* edited message, default is \`1\` (newest), max is \`${require('../modules/sniper').maxSnipes}\` (oldest) \n\`[channel]\`: channel to snipe`)
  .setSimilar('`$$snipe` `$$reactionsnipe`')
  .requiredPerm('User')
  .isDank()
  .applicationOptions([
