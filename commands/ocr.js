...
module.exports = new BotCmd('ocr')
  .setCategory('Media')
  .setDescription('Get text from an image')
  .setUsage('Enter `$$ocr` and add links or attach an image, or reply `$$ocr` to a message')
  .setAliases(['text'])
  .requiredPerm('User')
  .applicationOptions([{ type: 3, name: 'link', description: 'Image link (if many, separate with a space)', required: true }])
